...
async function api(url, options = {}) {
  ...
  const endpoint = url.startsWith("http") ? url : BACKEND_URL + url
...
function loginSuccess(data) {
  ...
  loggedInUserId = data.user?._id || data.userId
...
function renderSingleMessage(message, element) {
  ...
  if (message.deleted) {
    ...
    content.innerHTML = `
      <img class="messageImage" src="${BACKEND_URL}/api/media/${message.mediaId}" alt="${escapeHtml(message.mediaName)}">
      ${message.text ? `<div>${escapeHtml(message.text)}</div>` : ""}
    `
  } else if (message.type === "video") {
    content.innerHTML = `
      <video class="messageVideo" controls src="${BACKEND_URL}/api/media/${message.mediaId}"></video>
      ${message.text ? `<div>${escapeHtml(message.text)}</div>` : ""}
    `
  } else if (message.type === "audio") {
    content.innerHTML = `
      <audio class="messageAudio" controls src="${BACKEND_URL}/api/media/${message.mediaId}"></audio>
    `
  } else if (message.type === "document") {
    content.innerHTML = `
      <div class="messageFile">
        <a href="${BACKEND_URL}/api/media/${message.mediaId}" target="_blank" rel="noopener noreferrer" style="color: #38bdf8; text-decoration: underline;">
...
function setupEventListeners() {
  safeOn("registerForm", "submit", async event => {
    ...
    if (!validateCollegeEmail(email)) {
      alert("Only @akgec.ac.in college email addresses are allowed")
      return
    }
    ...
    try {
      ...
      if (msg) msg.textContent = "Registration successful! Logging you in..."
