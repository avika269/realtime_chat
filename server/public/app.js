...
async function loadUsers(search = "") {
  ...
  try {
    ...
    if (!Array.isArray(users) || users.length === 0) {
      container.innerHTML = `
        <div style="padding: 1.5rem 1rem; text-align: center; color: #94a3b8; font-size: 0.9rem;">
          No other users found.<br>
          <small style="opacity: 0.7;">Create another account in Incognito to start chatting.</small>
        </div>
      `
      return
    }
...
function renderSingleMessage(message, element) {
  ...
  if (message.replyTo) {
    ...
    reply.textContent = `Reply: ${message.replyTo.text || message.replyTo.mediaName || "Media"}`
  ...
  if (message.deleted) {
    ...
    content.innerHTML = `
      <div class="messageFile">
        <a href="/api/media/${message.mediaId}" target="_blank" rel="noopener noreferrer">
...
async function openUserProfile() {
  ...
  try {
    ...
    if (aboutEl) aboutEl.textContent = user.about || "No status available"
    if (statusEl) {
  ...
  try {
    ...
    calls.forEach(call => {
      ...
      element.innerHTML = `
        ...
        <span>${call.type} call · ${call.status}</span>
      `
      container.appendChild(element)
    })
  } catch (error) {
